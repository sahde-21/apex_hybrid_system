<x-layouts.supplier-auth>
    <div class="space-y-6">
        <div class="space-y-1 text-center">
            <flux:heading size="lg">{{ __('scf.supplier_portal.two_factor_title') }}</flux:heading>
            <flux:subheading>{{ __('scf.supplier_portal.two_factor_subtitle') }}</flux:subheading>
        </div>

        <form method="POST" action="{{ route('supplier.two-factor.login.store') }}" class="space-y-4">
            @csrf
