<x-layouts.supplier-auth>
    <div class="space-y-6">
        <div class="space-y-1 text-center">
            ...
            <flux:subheading>{{ __('scf.supplier_portal.forgot_subtitle') }}</flux:subheading>
        </div>
        @if (session('status'))
            <flux:callout variant="success">{{ session('status') }}</flux:callout>
        @endif
        <form method="POST" action="{{ route('supplier.password.email') }}" class="space-y-4">
            @csrf
            ...
            <div class="text-center">
                <flux:link :href="route('supplier.login')">{{ __('scf.supplier_portal.back_to_login') }}</flux:link>
            </div>
        </form>
    </div>
</x-layouts.supplier-auth>
