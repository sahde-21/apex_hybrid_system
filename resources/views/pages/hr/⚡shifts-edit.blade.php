...
new #[Title('Edit Shift management')] class extends \App\Livewire\ConcernBases\ShiftValidationRulesBase {
    ...
    {
        $validated = $this->validate($this->shiftUpdateRules($this->shift->id));

        $this->shift->update($validated);

        Flux::toast(variant: 'success', text: __('Shift management updated successfully.'));
...
<section class="w-full">
    <div class="mb-6">
        <flux:heading size="xl">{{ __('Edit Shift management') }}</flux:heading>
    </div>
