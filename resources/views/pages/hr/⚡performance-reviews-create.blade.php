...
new #[Title('Create Performance reviews')] class extends \App\Livewire\ConcernBases\PerformanceReviewValidationRulesBase {
    ...
    {
        $validated = $this->validate($this->performanceReviewRules());

        PerformanceReview::query()->create($validated);

        Flux::toast(variant: 'success', text: __('Performance reviews created successfully.'));
...
<section class="w-full">
    <div class="mb-6">
        <flux:heading size="xl">{{ __('Create Performance reviews') }}</flux:heading>
    </div>
