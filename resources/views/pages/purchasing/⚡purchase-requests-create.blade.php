...
new #[Title('Create purchase request')] class extends Component {
    ...
    public function save(): void
    {
        ...
        try {
            $pr = app(PurchaseRequestWorkflowService::class)->create(auth()->user(), [
            ...
            Flux::toast(variant: 'success', text: __('scf.purchase_workflow.pr_created'));
            $this->redirect(route('purchase-requests.show', $pr), navigate: true);
...
<section class="scf-page">
    <div class="mb-6 flex items-start justify-between">
        <div>
            <flux:heading size="xl">{{ __('Create purchase request') }}</flux:heading>
            <flux:subheading>{{ __('Record an internal purchase request') }}</flux:subheading>
        </div>
        <flux:button :href="route('purchase-requests.index')" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="scf-card grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <flux:input wire:model="department" :label="__('Department')" :placeholder="__('e.g. Operations')" />
        ...
        <div class="flex gap-2">
            <flux:button type="submit" variant="primary">{{ __('Create request') }}</flux:button>
            <flux:button :href="route('purchase-requests.index')" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
        </div>
    </form>
</section>
