...
new #[Title('Edit purchase order')] class extends Component {
    ...
    public function mount(PurchaseOrder $purchaseOrder): void
    {
        ...
        if (! $purchaseOrder->status->isEditable()) {
            $this->redirect(route('purchase-orders.show', $purchaseOrder), navigate: true);
            return;
        }
    ...
    public function save(): void
    {
        ...
        try {
            app(PurchaseOrderWorkflowService::class)->update($this->purchaseOrder, auth()->user(), [
            ...
            Flux::toast(variant: 'success', text: __('scf.purchase_workflow.order_updated'));
            $this->redirect(route('purchase-orders.show', $this->purchaseOrder), navigate: true);
...
<section class="scf-page">
    <div class="mb-6 flex items-start justify-between">
        <div>
            ...
            <flux:subheading>{{ $purchaseOrder->reference_number }}</flux:subheading>
        </div>
        <flux:button :href="route('purchase-orders.show', $purchaseOrder)" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
    </div>
