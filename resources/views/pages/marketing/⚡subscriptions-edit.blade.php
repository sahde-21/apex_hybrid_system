...
new #[Title('Edit Subscriptions')] class extends \App\Livewire\ConcernBases\SubscriptionValidationRulesBase {
    ...
    {
        $validated = $this->validate($this->subscriptionUpdateRules($this->subscription->id));

        $this->subscription->update($validated);

        Flux::toast(variant: 'success', text: __('Subscriptions updated successfully.'));
...
<section class="w-full">
    <div class="mb-6">
        <flux:heading size="xl">{{ __('Edit Subscriptions') }}</flux:heading>
    </div>
