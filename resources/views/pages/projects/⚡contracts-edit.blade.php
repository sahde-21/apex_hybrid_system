...
new #[Title('Edit Contracts')] class extends \App\Livewire\ConcernBases\ContractValidationRulesBase {
    ...
    public function mount(Contract $contract): void
    {
        ...
        $this->reference_number = $contract->reference_number ?? '';
    ...
    {
        $validated = $this->validate($this->contractUpdateRules($this->contract->id));

        $this->contract->update($validated);

        Flux::toast(variant: 'success', text: __('Contracts updated successfully.'));
...
<section class="w-full">
    <div class="mb-6">
        <flux:heading size="xl">{{ __('Edit Contracts') }}</flux:heading>
    </div>
