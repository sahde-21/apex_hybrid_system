...
new #[Title('Edit sale order')] class extends Component {
    ...
    public function mount(SaleOrder $saleOrder): void
    {
        ...
        if (! $saleOrder->status->isEditable()) {
            $this->redirect(route('sale-orders.show', $saleOrder), navigate: true);
            return;
        }

        $this->saleOrder = $saleOrder->load('lines.product');
        ...
        $this->currency_code = $saleOrder->currency_code;
        $this->notes = $saleOrder->notes ?? '';
        $this->billing_address = $saleOrder->billing_address ?? '';
        $this->shipping_address = $saleOrder->shipping_address ?? '';

        $this->lines = $saleOrder->lines->map(fn ($line) => [
    ...
    public function save(): void
    {
        ...
        try {
            $order = app(SaleOrderWorkflowService::class)->update($this->saleOrder, auth()->user(), [
...
<section class="scf-page">
    <div class="mb-6 flex items-start justify-between">
        <div>
            ...
            <flux:subheading>{{ $saleOrder->reference_number }}</flux:subheading>
        </div>
        <flux:button :href="route('sale-orders.show', $saleOrder)" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="scf-card grid gap-4 sm:grid-cols-2">
            ...
            <flux:textarea wire:model="shipping_address" :label="__('Shipping address')" rows="3" />
