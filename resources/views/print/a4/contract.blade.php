...
<head>
    <meta charset="utf-8">
    <title>{{ __('scf.contracts') }} {{ $record->reference_number }}</title>
    <style>
...
<div class="sheet">
    <div class="brand">
        ...
        <div class="doc-type">{{ __('scf.contracts') }}</div>
    </div>
    ...
    <div class="meta">
        <div class="box">
            ...
            <p>{{ $record->contact?->company_name }}</p>
        </div>
        <div class="box">
            <h3>{{ __('scf.reference') }}</h3>
            <p><strong>{{ $record->reference_number }}</strong></p>
            <p>{{ __('Title') }}: {{ $record->title }}</p>
            <p>{{ __('Start') }}: {{ $record->start_date?->format('Y-m-d') }}</p>
            <p>{{ __('End') }}: {{ $record->end_date?->format('Y-m-d') }}</p>
            <p>{{ __('scf.status') }}: {{ $record->status?->label() ?? $record->status }}</p>
            <p>{{ __('Value') }}: {{ number_format((float) $record->value, 2) }}</p>
        </div>
    </div>
