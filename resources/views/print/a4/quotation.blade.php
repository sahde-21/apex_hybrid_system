...
<body onload="window.print()">
    <div class="brand">
        ...
        <div class="doc-type">{{ __('scf.quotations') }}</div>
    </div>
    <div class="meta">
        <div class="box">
        ...
        <div class="box">
            ...
            <p>{{ __('scf.date') }}: {{ $record->quotation_date?->format('Y-m-d') }}</p>
            <p>{{ __('scf.valid_until') }}: {{ $record->valid_until?->format('Y-m-d') ?? '—' }}</p>
    ...
    @if($record->notes)
        <p>{{ $record->notes }}</p>
    @endif
    <div class="totals">{{ __('scf.total') }}: {{ number_format((float) $record->total_amount, 2) }}</div>
