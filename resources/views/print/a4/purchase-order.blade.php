...
<head>
    <meta charset="utf-8">
    <title>{{ __('scf.purchase_orders') }} {{ $record->reference_number }}</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .doc-type { font-size: 22px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #334155; }
        ...
        .totals { margin-top: 20px; margin-inline-start: auto; width: 260px; font-size: 16px; font-weight: 700; display: flex; justify-content: space-between; border-top: 2px solid #0f172a; padding-top: 10px; }
...
<div class="sheet">
    <div class="brand">
        ...
        <div class="doc-type">{{ __('scf.purchase_orders') }}</div>
    </div>
    ...
    <div class="meta">
        <div class="box">
            ...
            <p>{{ __('scf.warehouse_label') }}: {{ $record->warehouse?->name ?? '—' }}</p>
        </div>
        <div class="box">
            <h3>{{ __('scf.reference') }}</h3>
            <p><strong>{{ $record->reference_number }}</strong></p>
            <p>{{ __('scf.order_date') }}: {{ $record->order_date?->format('Y-m-d') }}</p>
            <p>{{ __('scf.expected') }}: {{ $record->expected_date?->format('Y-m-d') ?? '—' }}</p>
    ...
    <div class="totals">
        <span>{{ __('scf.total') }}</span>
        <span>{{ number_format((float) $record->total_amount, 2) }}</span>
    </div>
