...
<body onload="window.print()">
    <div class="center">
        ...
        <div class="muted">{{ __('scf.sale_orders') }}</div>
    </div>
    <div class="line"></div>
    <div class="row"><span>{{ __('scf.reference') }}</span><span>{{ $record->reference_number }}</span></div>
    <div class="row"><span>{{ __('scf.order_date') }}</span><span>{{ $record->order_date?->format('Y-m-d') }}</span></div>
    <div class="row"><span>{{ __('scf.delivery') }}</span><span>{{ $record->delivery_date?->format('Y-m-d') ?? '—' }}</span></div>
    <div class="row"><span>{{ __('scf.customer') }}</span><span>{{ $record->contact?->name ?? '—' }}</span></div>
    <div class="row"><span>{{ __('scf.warehouse_label') }}</span><span>{{ $record->warehouse?->name ?? '—' }}</span></div>
