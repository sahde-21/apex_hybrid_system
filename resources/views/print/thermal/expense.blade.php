...
<body onload="window.print()">
    <div class="center">
        ...
        <div class="muted">{{ __('scf.expenses') }}</div>
    </div>
    <div class="line"></div>
    <div class="row"><span>{{ __('scf.reference') }}</span><span>{{ $record->reference_number }}</span></div>
    <div class="row"><span>{{ __('scf.date') }}</span><span>{{ $record->expense_date?->format('Y-m-d') }}</span></div>
    <div class="row"><span>{{ __('scf.category') }}</span><span>{{ $record->category }}</span></div>
    <div class="row"><span>{{ __('scf.payment_method') }}</span><span>{{ $record->payment_method ?? '—' }}</span></div>
