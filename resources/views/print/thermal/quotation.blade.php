...
<head>
    <meta charset="utf-8">
    <title>{{ __('scf.quotations') }} {{ $record->reference_number }}</title>
    <style>
...
<body onload="window.print()">
    <div class="center">
        ...
        <div class="muted">{{ __('scf.quotations') }}</div>
    </div>
    <div class="line"></div>
    <div class="row"><span>{{ __('scf.reference') }}</span><span>{{ $record->reference_number }}</span></div>
    <div class="row"><span>{{ __('scf.date') }}</span><span>{{ $record->quotation_date?->format('Y-m-d') }}</span></div>
    <div class="row"><span>{{ __('scf.valid_until') }}</span><span>{{ $record->valid_until?->format('Y-m-d') ?? '—' }}</span></div>
